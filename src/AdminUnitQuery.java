...
public class AdminUnitQuery {
    AdminUnitList src;
    Predicate<AdminUnit> p = a->true; // domyślnie każdy spełnia
    Comparator<AdminUnit> cmp = (a,b) -> 1; // domyślny komparator, żeby nie było błędów dla braku
    ...
    AdminUnitList execute(){
        AdminUnitList result;
        //te ktore spelniaja predykat
        //sortujemy
        //obcinamy
        result = src.filter(p).
                sort(cmp).
                filter(a -> true,offset,limit);//bierzemy wszystkie po offsecie dlatego true
        return result;
    }
}